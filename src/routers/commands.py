...
async def all_other_messages_handler(message: Message) -> None:
    excel_file_path = get_excel_filename_by_chat_id(message)

    if not await send_document_if_exists(message, excel_file_path):
        await message.answer('Вы ещё не сохраняли документ!')
